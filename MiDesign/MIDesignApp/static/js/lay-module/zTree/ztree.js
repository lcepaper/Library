 ...
 // debugger;
  var settings = {}, roots = {}, caches = {},
    ...
    tools = {
      ...
      o: function (node, exp, setting) {
        if (!!exp && typeof exp != "string") {
          setting = exp;
          exp = "";
        }
        if (typeof node == "string") {
          return o(node, setting ? setting.treeObj.get(0).ownerDocument : null);
        } else {
          return o("#" + node.tId + exp, setting ? setting.treeObj : null);
        }
      },
      ...
      getNodeMainDom: function (target) {
        return (o(target).parent("li").get(0) || o(target).parentsUntil("li").parent().get(0));
      },
      isChildOrSelf: function (dom, parentId) {
        return (o(dom).closest("#" + parentId).length > 0);
      },
    ...
    view = {
      addNodes: function (setting, parentNode, index, newNodes, isSilent) {
        ...
        if (parentNode) {
          var target_switchObj = oo(parentNode, consts.id.SWITCH, setting),
            target_icoObj = oo(parentNode, consts.id.ICON, setting),
            target_ulObj = oo(parentNode, consts.id.UL, setting);
      ...
      appendParentULDom: function (setting, node) {
        var html = [],
          nObj = oo(node, setting);
        if (!nObj.get(0) && !!node.parentTId) {
          view.appendParentULDom(setting, node.getParentNode());
          nObj = oo(node, setting);
        }
        var ulObj = oo(node, consts.id.UL, setting);
        if (ulObj.get(0)) {
          ulObj.remove();
        }
        var children = data.nodeChildren(setting, node),
      ...
      asyncNode: function (setting, node, isSilent, callback) {
        ...
        if (node) {
          node.isAjaxing = true;
          var icoObj = oo(node, consts.id.ICON, setting);
      ...
      cancelPreSelectedNode: function (setting, node, excludeNode) {
        ...
        for (i = list.length - 1; i >= 0; i--) {
          ...
          if (node === n || (!node && (!excludeNode || excludeNode !== n))) {
            oo(n, consts.id.A, setting).removeClass(consts.node.CURSELECTED);
            if (node) {
      ...
      createNodes: function (setting, level, nodes, parentNode, index) {
        ...
        var root = data.getRoot(setting),
          openFlag = !parentNode || parentNode.open || !!oo(data.nodeChildren(setting, parentNode)[0], setting).get(0);
        root.createdNodes = [];
        ...
        if (!parentNode) {
          ...
          var ulObj = oo(parentNode, consts.id.UL, setting);
          if (ulObj.get(0)) {
            parentObj = ulObj;
        ...
        if (parentObj) {
          ...
          if (index >= 0 && nextObj) {
            o(nextObj).before(zTreeHtml.join(''));
          } else {
      ...
      expandCollapseNode: function (setting, node, expandFlag, animateFlag, callback) {
        ...
        if (!node.open && isParent && ((!oo(node, consts.id.UL, setting).get(0)) || (children && children.length > 0 && !oo(children[0], setting).get(0)))) {
        ...
        var ulObj = oo(node, consts.id.UL, setting),
          switchObj = oo(node, consts.id.SWITCH, setting),
          icoObj = oo(node, consts.id.ICON, setting);
      ...
      removeChildNodes: function (setting, node) {
        ...
        if (!setting.data.keep.parent) {
          ...
          node.open = false;
          var tmp_switchObj = oo(node, consts.id.SWITCH, setting),
            tmp_icoObj = oo(node, consts.id.ICON, setting);
          view.replaceSwitchClass(node, tmp_switchObj, consts.folder.DOCU);
          view.replaceIcoClass(node, tmp_icoObj, consts.folder.DOCU);
          oo(node, consts.id.UL, setting).remove();
        } else {
          oo(node, consts.id.UL, setting).empty();
        }
      },
      ...
      removeNode: function (setting, node) {
        ...
        if (!setting.data.keep.parent && childLength == 0) {
          ...
          tmp_ulObj = oo(parentNode, consts.id.UL, setting);
          tmp_switchObj = oo(parentNode, consts.id.SWITCH, setting);
          tmp_icoObj = oo(parentNode, consts.id.ICON, setting);
          ...
          tmp_ulObj = oo(newLast, consts.id.UL, setting);
          tmp_switchObj = oo(newLast, consts.id.SWITCH, setting);
          tmp_icoObj = oo(newLast, consts.id.ICON, setting);
          if (parentNode == root) {
            if (children.length == 1) {
              //node was root, and ztree has only one root after move node
              view.replaceSwitchClass(newLast, tmp_switchObj, consts.line.ROOT);
            } else {
              var tmp_first_switchObj = oo(children[0], consts.id.SWITCH, setting);
      ...
      selectNode: function (setting, node, addFlag) {
        ...
        oo(node, consts.id.A, setting).addClass(consts.node.CURSELECTED);
        data.addSelectedNode(setting, node);
        setting.treeObj.trigger(consts.event.SELECTED, [setting.treeId, node]);
      },
      setNodeFontCss: function (setting, treeNode) {
        var aObj = oo(treeNode, consts.id.A, setting),
      ...
      setNodeLineIcos: function (setting, node) {
        if (!node) return;
        var switchObj = oo(node, consts.id.SWITCH, setting),
          ulObj = oo(node, consts.id.UL, setting),
          icoObj = oo(node, consts.id.ICON, setting),
      ...
      setNodeName: function (setting, node) {
        var title = data.nodeTitle(setting, node),
          nObj = oo(node, consts.id.SPAN, setting);
        nObj.empty();
        ...
        if (tools.apply(setting.view.showTitle, [setting.treeId, node], setting.view.showTitle)) {
          var aObj = oo(node, consts.id.A, setting);
  ...
  o.fn.zTree = {
    ...
    init: function (obj, zSetting, zNodes) {
      ...
      o.extend(true, setting, zSetting);
      ...
      var zTreeTools = {
        ...
        expandNode: function (node, expandFlag, sonSign, focus, callbackFlag) {
          ...
          function showNodeFocus() {
            var a = oo(node, consts.id.A, setting).get(0);
            if (a && focus !== false) {
