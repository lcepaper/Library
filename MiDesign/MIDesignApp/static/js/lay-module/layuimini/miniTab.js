...
layui.define(["element", "layer", "jquery"], function (exports) {
    ...
    var miniTab = {
        ...
        create: function (options) {
            ...
            options.flush=options.flush||false;
            options.isIframe = false;
            options.maxTabNum = options.maxTabNum || 20;


            if (!options.flush&&$(".layuimini-tab .layui-tab-title li").length >= options.maxTabNum) {
            ...
            $.ajax({
                ...
                success: function (result) {
                                ...
                                yes: function (index, layero) {
                                    ...
                                    $.post("/ajaxlogin", {
                                        ...
                                        if (data.status != 200) {
                                            ...
                                            if(options.href=="/admin")
                                            {
                                                location.href='/admin'
                                                return;
                                            }
                                            layer.close(index)
                                            miniTab.create(options);
                                        }
                                    }, "json")
                    ...
                    {
                    $(".layui-tab-item.layui-show").html(result);
                    }
                    ele.tabChange('layuiminiTab', options.tabId)
                },
                error: function (data) {
                    ...
                    {
                        $(".layui-tab-item.layui-show").html(data);
                    }
                    ele.tabChange('layuiminiTab', options.tabId)
                }
            });
        ...
        listenHash: function (options) {
            ...
            } else if(tabId === '/characterization'){
                menu = {
                    href: tabId,
                    title: '实验表征管理',
                    tabId: tabId,
